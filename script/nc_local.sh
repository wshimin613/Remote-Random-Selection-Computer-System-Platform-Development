...
username=${2}
IP=${1}
res1_Ac=`cat <(echo "cmd.exe /c C:\users\public\query.exe user ${username} | find \"Active\"") <(sleep 0.5) |nc -w 1 -n ${IP} 3501 | grep Active | grep -v find`
...
if [ "$res1_Ac" ];then
	...
	res2_Ac=`cat <(echo "cmd.exe /c C:\users\public\query.exe user ${username} | find \"Active\"") <(sleep 0.5) |nc -w 1 -n ${IP} 3502 | grep Activei | grep -v find`
	...
	if [ "$res2_Ac" ];then
		...
		res3_Ac=`cat <(echo "cmd.exe /c C:\users\public\query.exe user ${username} | find \"Active\"") <(sleep 0.5) |nc -w 1 -n ${IP} 3503 | grep Active | grep -v find`
