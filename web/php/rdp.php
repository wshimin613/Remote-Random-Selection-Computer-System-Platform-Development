<?php
	...
	$res = $db->query("$Q");
    // status 0. 沒有使用 1. 使用中 2. 實體機開機中 3. 虛擬機開機中 4. 確認機器中 5. 機器開啟錯誤 6. 沒有機器可用
			...
			while( $i == 0 ){
				...
				if ( ($res1->num_rows && $res2->num_rows) ||  $vm == '' ){
					...
					if ( $j == 2 ){
						...
						exec("sudo /bin/sh /root/script/min/autocreate3.sh $username $userIP $userport");
						$i=5;
					}
				}
				else {
